...
func writeEntriesToFile(entries []kdnode.Key) {
	data, err := json.Marshal(entries)
	if err != nil {
		fmt.Println(Red, "Error marshaling entries:", err, Reset)
		return
	}
	err = ioutil.WriteFile("entries.json", data, 0644)
	if err != nil {
		fmt.Println(Red, "Error writing entries to file:", err, Reset)
	}
}
